...
const Header = () => {
  const [user, setUser] = useState<string>('GuestAccount');
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
      if (currentUser) {
        setUser(currentUser.displayName || currentUser.email || currentUser.uid);
      } else {
      ...
      <div className='max-w-5xl mx-auto py-2 flex justify-between'>
        ...
        <div className='text-xl mt-2.5'>
          {user !== 'GuestAccount'?(
            ...
            <div>
              <Link href="/login" className='border border-black rounded-full px-5 py-0.5 bg-white' >LogIn</Link>
              <Link href="/signup" className='border border-black rounded-full px-5 py-0.5 ml-5 bg-white'>SignUp</Link>
              <div className= 'ml-3 mt-1'>{user}</div>
            </div>
          )}  
        </div>
